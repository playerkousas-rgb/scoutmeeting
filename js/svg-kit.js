...
/* ══════════ 👕 制服：徽章佩戴位置（v31）══════════
   ...
   圖上只寫短標記（①–⑨），詳細名稱由 app.js 用 HTML 列喺圖下面（好讀又唔會出框）。 */
D.uniform = {};

/* 胸袋上下層：袋蓋上方 3cm（上層）／袋蓋上方（下層）／袋中央
   圖內只放 ①–⑥ 標記，名稱留 HTML 表（避免文字出 viewBox） */
D.uniform.chest = svg(340,200,'童軍制服胸袋徽章層次示意圖', MK
  +T(170,14,'胸袋層次圖（正面睇）',11,'#1B5E20','middle',1)
  +box(96,22,148,150,'#F7E7C8','#C9A96A',8)
  +'<path d="M142,22 L170,48 L198,22" fill="none" stroke="#B08D57" stroke-width="2"/>'
  +box(108,88,52,36,'#FBF3E2','#B08D57',4)
  +ln(108,96,160,96,1.5,'#B08D57')
  +box(180,88,52,36,'#FBF3E2','#B08D57',4)
  +ln(180,96,232,96,1.5,'#B08D57')
  ...
  +T(170,182,'（同一層最多兩個，並行排列）',9,'#A1887F')
);

/* 全身位置：胸 ①–⑥＋袖 ⑦⑧＋專章帶 ⑨ */
D.uniform.body = svg(340,250,'童軍制服徽章佩戴全位置示意圖', MK
  +T(170,14,'全身佩戴位置（正面睇）',11,'#1B5E20','middle',1)
  +'<circle cx="170" cy="42" r="13" fill="#EFE0C8" stroke="#C9A96A" stroke-width="1.6"/>'
  +'<path d="M132,62 L208,62 L216,92 L216,226 L124,226 L124,92 z" fill="#F7E7C8" stroke="#C9A96A" stroke-width="2"/>'
  +'<path d="M132,62 L112,70 L104,124 L122,130 L124,92 z" fill="#F7E7C8" stroke="#C9A96A" stroke-width="2"/>'
...
/* 陸／海／空三組顏色配搭示意（本地圖，離線都睇得到；實際樣式以官網圖為準） */
function branchFig(title, rows){
  var body = MK + T(170,15,title+'：顏色配搭（示意）',11,'#1B5E20','middle',1);
  rows.forEach(function(r,i){
    var y = 32 + i*26;
    body += box(16,y,26,18,r[1],'#90A4AE',4)
      + T(52,y+13,r[0],10,'#455A64','start',1)
      + T(90,y+13,r[2],9.5,'#546E7A','start');
  });
  body += T(170,32+rows.length*26,'只示顏色同配搭；徽章位置睇「徽章佩戴」分頁嘅位置圖。',9,'#8D6E63');
  return svg(340, 44+rows.length*26, title+'顏色配搭示意圖', body);
}
D.uniform.land = branchFig('陸童軍（男／女）', [
  ['帽','#1B5E20','深綠色軟帽（連童軍帽章）'],
  ['恤','#F3E3C3','杏色短袖／長袖恤（兩胸袋、肩帶）'],
  ['褲裙','#7CB342','草青色短褲／裙褲'],
  ['襪','#8D6E63','深色長襪拉至膝下'],
  ['鞋','#4E342E','皮鞋擦亮、綁帶'],
  ['領巾','#00695C','深綠色旅巾＋巾圈']
]);
D.uniform.sea = branchFig('海童軍（男／女）', [
  ['帽','#ECEFF1','白色海童軍帽（連帽帶）'],
  ['恤','#FAFAFA','白色恤（深藍細節）'],
  ['褲裙','#1A237E','深藍色短褲／裙褲'],
  ['襪','#1A237E','深藍色長襪'],
...
D.uniform.air = branchFig('空童軍（男／女）', [
  ['帽','#78909C','灰藍色軟帽（連童軍帽章）'],
  ['恤','#B3E5FC','淺藍色恤'],
  ['褲裙','#1A237E','深藍色短褲／裙褲'],
  ['襪','#1A237E','深藍色長襪'],
  ['鞋','#4E342E','皮鞋'],
...
/* 領帶 4 色（按《儀容與制服手冊》3.4：棗紅／深綠／黑／深藍） */
function tieFig(x,y,col){
  return '<path d="M'+(x-5)+','+y+' L'+(x+5)+','+y+' L'+(x+7)+','+(y+9)+' L'+x+','+(y+12)+' L'+(x-7)+','+(y+9)+' z" fill="'+col+'" stroke="#37474F" stroke-width="1"/>'
    + '<path d="M'+x+','+(y+12)+' L'+(x+8)+','+(y+20)+' L'+(x+4)+','+(y+58)+' L'+x+','+(y+64)+' L'+(x-4)+','+(y+58)+' L'+(x-8)+','+(y+20)+' z" fill="'+col+'" stroke="#37474F" stroke-width="1"/>';
}
D.uniform.ties = svg(340,215,'領帶四種顏色示意圖', MK
  +T(170,14,'領帶 4 色（按支部／團屬）',11,'#1B5E20','middle',1)
  +tieFig(50,34,'#7B1E28')+T(50,112,'棗紅色',10,'#7B1E28','middle',1)+T(50,126,'深資童軍',9,'#546E7A')
  +tieFig(130,34,'#1B5E20')+T(130,112,'深綠色',10,'#1B5E20','middle',1)+T(130,126,'樂行＋成年',9,'#546E7A')
  +tieFig(210,34,'#212121')+T(210,112,'黑色',10,'#212121','middle',1)+T(210,126,'海童軍',9,'#546E7A')
  +tieFig(290,34,'#1A237E')+T(290,112,'深藍色',10,'#1A237E','middle',1)+T(290,126,'空童軍',9,'#546E7A')
  +T(170,146,'（深資海／空、樂行海／空跟所屬支部顏色；童軍支部成員一般戴領巾）',8.5,'#8D6E63')
  +ln(20,160,320,160,1,'#CFD8DC')
  +T(170,178,'佩戴：結要對稱（Windsor 等）・寬帶比窄帶長',9,'#455A64')
  +T(170,192,'寬帶尖觸及皮帶扣上端，唔可以超越皮帶扣・結套牢喺衣領尖',9,'#455A64')
);

/* 基維爾巾圈／木章皮繩位置（3.5） */
D.uniform.kilwell = svg(340,205,'基維爾木章皮繩佩戴位置示意圖', MK
  +T(170,14,'木章（皮繩連木珠）戴邊度',11,'#1B5E20','middle',1)
  /* 三個小圖：領巾制服／領帶制服／禮服 */
  +'<circle cx="56" cy="76" r="12" fill="#EFE0C8" stroke="#C9A96A" stroke-width="1.4"/>'
...
D.uniform.zoom = svg(340,280,'徽章位置局部放大圖：左胸袋與右袖肩膊', MK
  ...
  +T(92,84,'① 左胸袋（放大）',10,'#6A1B9A','middle',1)
  +box(48,104,88,66,'#F7E7C8','#C9A96A',6)
  +ln(48,120,136,120,1.4,'#B08D57')
...
/* 旅巾綁法（領巾規格：捲巾直徑 3.5cm、底至尖 12–15cm、巾尾唔超越皮帶扣） */
D.uniform.scarf = svg(340,215,'旅巾綁法與長度規格', MK
  +T(170,14,'旅巾綁法（巾圈套喺衣領尖）',11,'#1B5E20','middle',1)
  +'<path d="M120,40 Q170,20 220,40 L226,74 L114,74 z" fill="#E8F5E9" stroke="#2E7D32" stroke-width="1.6"/>'
  +'<circle cx="170" cy="52" r="9" fill="#FFFDF6" stroke="#2E7D32" stroke-width="1.4"/>'
  +T(170,55,'巾圈',7,'#2E7D32')
  ...
  +T(173,190,'皮帶線：巾尾唔可以超越皮帶扣',8.5,'#5D4037')
  +T(60,64,'①',11,'#C62828')+T(60,80,'捲巾',9,'#8D6E63')
  +T(60,110,'②',11,'#C62828')+T(60,126,'掛頸',9,'#8D6E63')
  +T(60,152,'③',11,'#C62828')+T(60,168,'套巾圈',9,'#8D6E63')
  +'<path d="M120,196 L150,196" stroke="#37474F" stroke-width="1.4" marker-end="url(#ah)"/>'
  +T(180,200,'巾圈收喺衣領尖、巾尾喺肚臍附近',8.5,'#37474F')
);
